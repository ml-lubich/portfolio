...
export const projects: Project[] = [
    ...
    },
    {
        ...
        detail: {
            ...
            gradient: g.accentToPrimary,
            accent: accentCycle[4],
            link: { label: "GitHub", url: "https://github.com/ml-lubich/imsg-mcp" },
        },
    },
    {
        id: "imail-mcp",
        ...
        summary:
            "macOS Mail.app CLI + MCP server with a guarded LLM auto-reply agent \u2014 drafts by default, auto-sends only high-confidence low-stakes replies to known contacts, backed by a labeled eval harness.",
        tags: ["CLI", "MCP", "AppleScript", "Python", "macOS"],
        gradient: g.cyanToPrimary,
        accent: accentCycle[2],
        number: "23",
        detail: {
            title: "imail-mcp",
            subtitle: "Apple Mail CLI + MCP with a guarded auto-reply agent",
            description:
                "imail is a CLI + built-in MCP server over Mail.app \u2014 accounts, list, send, organize, and `autodraft`, an LLM-drafted reply agent for the personal inbox. Every reply is a silent draft by default; a message only auto-sends when confidence \u2265 0.95, stakes are low, the sender is a known contact, the reply is short, there are no attachments, and the intent isn't recruiter \u2014 checks that read the mailbox, not the LLM's claim, which is also the prompt-injection defense. `autodraft-eval` runs a labeled case set against the real LLM and prints a scorecard before anything ships.",
            highlights: [
                "`autodraft` drafts by default; `auto_send_allowed()` is a pure gate \u2014 confidence \u2265 0.95, low stakes, known contact, \u2264 400 chars, no attachments, not recruiter intent",
                "`validate_decision()` fails closed on the untrusted LLM JSON output \u2014 wrong types or out-of-range confidence raise before a decision is used",
                "Built-in `imail mcp` FastMCP server over stdio, plus an `agent schema`/`agent guide` surface for LLM tool discovery",
                "`autodraft-eval` runs a labeled eval against the real LLM and prints a PASS/FAIL scorecard \u2014 no synthetic emails ever get sent",
            ],
            architecture: [
                { label: "CLI", icon: "layers", description: "imail \u2014 accounts, list, send, organize, autodraft, walls" },
                { label: "Auto-send gate", icon: "shield", description: "Deterministic confidence/stakes/contact/length/attachment checks, fail-closed" },
                { label: "Eval harness", icon: "server", description: "autodraft-eval scores labeled cases against the real LLM" },
                { label: "Mail.app bridge", icon: "cpu", description: "AppleScript automation over Mail.app \u2014 no IMAP client, no himalaya" },
            ],
            techStack: ["Python", "AppleScript", "Typer", "Mail.app", "Pydantic", "MCP", "pytest"],
            metrics: [
                { label: "Distribution", value: "PyPI: imail-mcp" },
                { label: "Type", value: "CLI + MCP + auto-reply agent" },
            ],
            diagramType: "cicd",
            gradient: g.cyanToPrimary,
            accent: accentCycle[2],
            link: { label: "GitHub", url: "https://github.com/ml-lubich/imail-mcp" },
        },
    },
    {
        id: "inotes-mcp",
    ...
    },
    {
        ...
        coverImage: "/images/projects/imessage-exporter-hero.png",
        summary:
            "TypeScript CLI and library that renders Markdown (including Mermaid diagrams) to a clean, print-ready PDF — tables that fit the page, wrapped code blocks, no mid-word breaks. npm i -g pdfify-md.",
        tags: ["TypeScript", "Node.js", "Markdown", "Mermaid", "PDF", "CLI"],
        gradient: g.primaryToRose,
    ...
    },
    {
        ...
        detail: {
            ...
            highlights: [
                ...
                "Published as mac-ical on PyPI and ml-lubich/tap/ical on Homebrew",
            ],
            architecture: [
                ...
                { label: "Distribution", icon: "git", description: "PyPI mac-ical and Homebrew ml-lubich/tap/ical" },
            ],
            techStack: ["Python", "AppleScript", "Typer", "Rich", "Calendar.app"],
            metrics: [
                { label: "Install", value: "pip install mac-ical" },
                { label: "Type", value: "CLI + MCP" },
            ],
            diagramType: "cicd",
            gradient: g.primaryToAccent,
    ...
    },
    {
        id: "vgate",
        name: "vgate",
        metric: "Multi-account Vercel CLI switcher",
        coverImage: "/images/projects/imessage-exporter-hero.png",
        summary:
            "vgate stores a full Vercel login (token, refresh token, expiry) per account and switches the active CLI login without vercel logout. pipx install vercel-mcp, or brew install ml-lubich/tap/vgate.",
        tags: ["CLI", "MCP", "Python", "Vercel"],
        gradient: g.primaryToCyan,
        accent: accentCycle[2],
        number: "31",
        detail: {
            title: "vgate",
            subtitle: "Switch Vercel CLI logins — CLI + MCP",
            description:
                "The Vercel CLI keeps one login for the whole machine. vgate saves each account's full credential and switches the active one, and vgate-mcp exposes the same commands to agents.",
            highlights: [
                "Keeps refresh token and expiry so an expired login is reported instead of a bare 401",
                "vgate switch takes an email, username, or row number",
                "MCP server vgate-mcp for agents",
                "PyPI vercel-mcp and Homebrew ml-lubich/tap/vgate",
            ],
            architecture: [
                { label: "CLI", icon: "layers", description: "vgate ls, switch, whoami, doctor" },
                { label: "Store", icon: "server", description: "Per-account token, refresh token, and expiry" },
                { label: "MCP", icon: "cpu", description: "vgate-mcp exposes the same commands to agents" },
                { label: "Distribution", icon: "git", description: "PyPI vercel-mcp and Homebrew tap" },
            ],
            techStack: ["Python", "Typer", "MCP", "Vercel CLI"],
            metrics: [
                { label: "Install", value: "brew install ml-lubich/tap/vgate" },
                { label: "Type", value: "CLI + MCP" },
            ],
            diagramType: "cicd",
            gradient: g.primaryToCyan,
            accent: accentCycle[2],
            link: { label: "GitHub", url: "https://github.com/ml-lubich/vercel-mcp" },
        },
    },
    {
        id: "claude-tiers",
        name: "claude-tiers",
        metric: "Model-tiered Claude Code ruleset",
        coverImage: "/images/projects/imessage-exporter-hero.png",
        summary:
            "Cheap, verified, hierarchical Claude Code — a plugin + ruleset that makes the frontier model the manager and cheaper models the workers, with evidence-based verification gates and an /overnight skill for unattended runs. Installable as a Claude Code plugin marketplace.",
        tags: ["Claude Code", "Plugin", "Markdown", "Agent Ops"],
        gradient: g.accentToPrimary,
        accent: accentCycle[4],
        number: "32",
        detail: {
            title: "claude-tiers",
            subtitle: "Model-tiered delegation ruleset + Claude Code plugin marketplace",
            description:
                "A tiny plugin + ruleset that routes work by tier: the frontier model plans and judges, a middle-manager model runs the orchestration loop, and cheaper worker models write the actual code — never grinding the expensive model on bulk edits. Built from a research pass over 2025–2026 Claude Code practitioner consensus, then audited against the academic literature with every rule graded VALIDATED/QUALIFIED/UNPROVEN/UNDERMINED and cited.",
            highlights: [
                "Tier ladder: frontier model plans + judges, Opus middle-manager orchestrates, Sonnet workers write code, Haiku handles search/renames/boilerplate",
                "Global ruleset template (~55 lines) covers manager→worker delegation, evidence-based 'done', 3-strikes on repeated errors, and screenshot-verified UI work",
                "/overnight skill for unattended multi-hour runs: checkpoint commits, crash-proof STATUS.md, blocked-phase handling, finish notification",
                "Doubles as a Claude Code plugin marketplace — /plugin marketplace add ml-lubich/claude-tiers",
            ],
            architecture: [
                { label: "Ruleset", icon: "layers", description: "templates/CLAUDE.md — manager/worker delegation and quality gates" },
                { label: "/overnight skill", icon: "cpu", description: "Unattended multi-hour runs with checkpoints and crash resume" },
                { label: "Research", icon: "server", description: "docs/research/ — practitioner consensus graded against arXiv literature" },
                { label: "Distribution", icon: "git", description: "Claude Code plugin marketplace, or copy the markdown by hand" },
            ],
            techStack: ["Markdown", "Claude Code Plugin"],
            metrics: [
                { label: "Install", value: "/plugin marketplace add ml-lubich/claude-tiers" },
                { label: "Type", value: "Ruleset + plugin" },
            ],
            diagramType: "pipeline",
            gradient: g.accentToPrimary,
            accent: accentCycle[4],
            link: { label: "GitHub", url: "https://github.com/ml-lubich/claude-tiers" },
        },
    },
]
