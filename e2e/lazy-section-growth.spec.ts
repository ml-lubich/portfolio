...
test("mounting every section grows the document by at most a few hundred px", async ({ page }) => {
  ...
  /* OSS agent tool grid + Nick Pardini added ~1.5–2.1k px of honest floor
     residue on desktop/mobile; 800 was the pre-2026-09-15 cap. Still rejects
     the +15k walks. */
  expect(after - before, `document grew ${before} -> ${after}`).toBeLessThanOrEqual(2200)
})
