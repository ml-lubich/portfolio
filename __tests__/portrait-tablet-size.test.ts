...
/* The intro portrait is aspect-[3/4] (full body with laptop) and only got a width cap at `lg:`. Between
 * sm and lg — iPad held upright — it therefore ran the full column width, so a
 * 768px-wide viewport rendered a tall frame and nothing else fit on screen. */
describe("profile intro portrait", () => {
    const source = read("components/sections/profile-intro.tsx")
    const frame = source.slice(source.indexOf("{/* Portrait"), source.indexOf("misha-desk-laptop"))
    ...
    it("keeps the 3:4 frame so object-cover never crops the image", () => {
        expect(frame).toContain("aspect-[3/4]")
    })
})
