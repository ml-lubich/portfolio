...
- `__tests__/oss-agent-tools.test.ts` — ten grid tools with short display names; `ossInstallFork()` leads with `brew tap ml-lubich/tap`. `__tests__/fork-page.test.ts` — `/fork` ships the fork block + grid. `e2e/oss-tool-grid.spec.ts` — grid visible on `#open-source`, click-to-copy works, `/fork` shows tap line.
