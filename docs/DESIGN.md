...
- **Nav row budget:** only links flagged `primary: true` in `components/nav/nav-links.ts` render inline (About, Journey, Consulting, Projects, Blog); every other section link lives in the desktop "More" dropdown, next to Tools and Games. The full list needs ~1500px, more than the shell gives, and at `2xl:` the CTA was clipped mid-word. Before you promote a link to `primary`, run `e2e/nav-fit.spec.ts`. The `nav-island-in` reveal must use fill-mode `backwards`: a retained end-state `clip-path` keeps clipping the shell, which cuts off the CTA and hides the dropdown panels. From `xl` up the shell is `w-fit` and hugs logo + links (centred, `gap-6`) instead of stretching edge to edge; `nav-fit.spec.ts` fails if more than 96px of empty space opens up inside it.
...
The same bug hid the main terminal's chrome (`components/terminal/index.tsx`:
window, header, footer, dots) and six blog glows (filter pills, tags, card
hover, avatar, share buttons). The terminal now uses inline styles; the glows
are plain CSS classes in `app/globals.css` (`.glow-blog-*`, `.glow-filter-*`)
on `hsl(var(--primary))` / `hsl(var(--accent))`, so they follow the theme. The
unused `shadows.*` tokens were removed from `lib/theme.ts`.
